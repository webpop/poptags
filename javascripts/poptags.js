(function() {
  ...
  breakFn = function(c, separator, last) {
    if (!/^[a-zA-Z]+$/.test(separator)) return separateFn(c, separator, last);
  ...
  separateFn = function(c, separator, last) {
    ...
    return function(value, index) {
      if (index === end) separator = last || separator;
      if (index === 0) {
        return value;
      } else {
  ...
  Ast = (function() {
    ...
    Ast.prototype.end_document = function(parser) {
      if (this.current_tag === this.root) {
        if (this.layout) return this._handle_layout(parser, this.layout);
    ...
    Ast.prototype.start_tag = function(parser, name, options) {
      ...
      if (name === CONSTANTS.BLOCK_TAG) {
        if (!this.layout) this.block_tag_without_layout(parser, name);
        this.layout.blocks[options.region] = new_tag;
        return this.current_tag = new_tag;
      } else {
        if (name === CONSTANTS.LAYOUT_TAG) {
          if (this.layout) this.layout_already_defined(parser, name);
          this.layout = new_tag;
        }
        if (options && this.filters) {
          for (key in options) {
            ...
            _ = options[key];
            if (this.filters[key]) new_tag.add_filter(this.filters[key]);
          }
        }
  ...
  Parser = (function() {
    ...
    Parser.prototype.next_tag_index = function(next_start, next_end, next_comment) {
      if (next_start < 0 && next_end < 0 && next_comment < 0) return -1;
        ...
        if (template_chunk.indexOf(CONSTANTS.COMMENT_TAG) === 0) {
          ...
          if (tag_attributes) {
            tag_attributes.replace(CONSTANTS.ATTRIBUTES_RE, function(match, name, doublequoted_value, singlequoted_value) {
              var value;
              if ((value = doublequoted_value || singlequoted_value).match(CONSTANTS.CONTAINS_TAGS_RE)) {
  ...
  Tag = (function(_super) {
    ...
    Tag.prototype.get_value = function() {
      var tag;
      if (this.module) this.module_scope = this.require(this.module);
      tag = this;
    ...
    Tag.prototype.tags = function(filter) {
      var tag, tags, _i, _j, _len, _len2, _ref, _ref2, _results, _results2;
      tags = [];
      if (filter) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          tag = _ref[_i];
          if (tag.render && filter(tag)) _results.push(tag);
        }
        return _results;
      } else {
        _ref2 = this.enclosing.collection;
        _results2 = [];
    ...
    Tag.prototype.render_function = function(value) {
      ...
      this.scope.lookup = function(name) {
        ...
        tag = self;
        while (tag) {
          if (tag.scope[name]) return tag.scope[name];
          tag = tag.parent;
        }
      };
      result = value.call(this._tag_fn_scope || value, options, enclosing_wrapper, this.scope);
      if (this.value_wrapper) result = this.value_wrapper.wrap(result, this);
    ...
    Tag.prototype.render_value = function(value) {
      ...
      if (value === true || value === false) return this.render_boolean(value);
      if (value && value.call) return this.render_function(value);
    ...
    Tag.prototype.render = function(scope) {
      ...
      if (this.region_tag) return this.render_region(this.get_block());
